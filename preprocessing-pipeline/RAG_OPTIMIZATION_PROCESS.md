...
| **[E]** | **コードブロック分離とタグ付け** | テキスト解説（`prose`）とコードサンプル（`code`: C++, Lua, Java等）を自動判定し、`chunk_type: "code"` メタデータと `code_lang` 情報を付与。 | ✅ **実装済み** `code_block_pattern = re.compile(r"```(\w*)\n(.*?)```")` でコード抽出・分離済み (`process_file()`) |
...
| **[L]** | **予想質問のメタデータ化 (Hypothetical Questions)** | LLMを用いて各チャンクに対して「ユーザーが探しそうな質問文」を2〜3件自動生成し、チャンクのメタデータ/埋め込み対象に加える。 | ❌ **未実装** 前処理パイプライン (`build_vdb_dataset.ipynb`) にLLMによる質問生成コードなし。`kanzi_rag_chunks.json` のメタデータスキーマにも `questions` フィールドなし。 |
