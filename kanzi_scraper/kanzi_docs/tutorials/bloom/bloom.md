---
title: Tutorial: Create a bloom effect
source: https://docs.kanzi.com/4.1.0/en/tutorials/bloom/bloom.html
---

# Tutorial: Create a bloom effect


In this tutorial, you learn how to use render passes to apply a bloom effect on 3D content. You create the bloom effect and apply it to the headlights of a car.

This video shows the result of the tutorial.

This tutorial assumes that you understand the basics of working with Kanzi Studio. The best entry points for getting familiar with Kanzi Studio are:

- Tutorial: Getting started with Kanzi Studio
- Tutorial: Create a simple in-vehicle infotainment application
- Tutorial: Materials and textures


Start with the tutorial
