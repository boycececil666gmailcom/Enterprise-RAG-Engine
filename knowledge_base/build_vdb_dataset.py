...
def process_file(filepath: Path, docs_root: Path, seen_hashes: set) -> list[dict]:
    ...
    for sec in sections:
        ...
        for cb_match in code_block_pattern.finditer(sec_content):
            ...
            # Add code block as distinct code chunk [Feature E + Context Enrichment]
            if len(code_content) >= 15:
                # Prepend Contextual Prefix
...
def _subchunk_prose(text: str, section_path: str, page_title: str, source_url: str, rel_path: str, seen_hashes: set) -> list[dict]:
    """Helper to split prose text into overlapping target-sized chunks with contextual headers."""
    prose_chunks = []
    ...
    for st in sub_texts:
        ...
        # Prepend Contextual Prefix [Anthropic Contextual Retrieval Pattern]
        ctx_prefix = f"[Document Context: {section_path}]" if section_path else f"[Document Context: {page_title}]"
