...
def ingest_chunks_via_http(chunks: list[dict], endpoint_url: str, batch_size: int = 50) -> dict:
    ...
    with httpx.Client(timeout=60.0) as client:
        for idx, chunk in enumerate(chunks, start=1):
            ...
            # Normalize metadata values (lists/dicts to JSON strings) for strict string metadata compatibility
            clean_metadata = {}
...
def main():
    ...
    parser.add_argument(
        "--input", "-i",
        type=str,
        default="rag_chunks.json.gz",
        help="Path to structured JSON or JSON.GZ dataset file (default: rag_chunks.json.gz)"
    )
    parser.add_argument(
        "--endpoint", "-e",
        type=str,
    ...
    # Search fallback locations if input file is not found in CWD
    if not input_path.exists():
        ...
        alt_paths = [
            ...
            cwd.parent / args.input,
            cwd.parent / "rag_chunks.json",
            cwd.parent / "rag_chunks.json.gz"
        ]
